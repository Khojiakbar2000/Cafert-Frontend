...
export default function ProductHeroSlider({ slides }: ProductHeroSliderProps) {
  ...
  useEffect(() => {
    ...
    const initApplication = async () => {
        ...
        const splitText = (text: string) => {
            return text.split('').map(char => `<span style="display: inline-block; opacity: 0; text-align: center;">${char === ' ' ? '&nbsp;' : char}</span>`).join('');
        };
        ...
        const updateContent = (idx: number) => {
            ...
            if (titleEl && descEl) {
                 ...
                 setTimeout(() => {
                     titleEl.innerHTML = splitText(slideData[idx].title);
                     descEl.textContent = slideData[idx].description; 
                     
                     // Content is now centered via flexbox wrapper, no need to set transforms
        ...
        const handleSlideChange = () => {
            ...
            navigateToSlide((currentSlideIndex + 1) % slideData.length);
        };
        ...
        const updateCounter = (idx: number) => { 
            ...
            const st = document.getElementById("slideTotal"); if (st) st.textContent = String(slideData.length).padStart(2, "0"); 
        };
        ...
        if (tEl && dEl) {
            tEl.innerHTML = splitText(slideData[0].title);
            dEl.textContent = slideData[0].description;
            // Content is centered via flexbox wrapper CSS
  ...
  return (
    <main className="slider-wrapper" ref={containerRef} style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden', margin: 0, padding: 0 }}>
      <canvas className="webgl-canvas" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }}></canvas>
      <span className="slide-number" id="slideNumber" style={{ position: 'absolute', top: '2rem', left: '2rem', zIndex: 10, color: 'white', fontSize: '1.5rem', fontFamily: 'monospace' }}>01</span>
      <span className="slide-total" id="slideTotal" style={{ position: 'absolute', top: '2rem', left: '5rem', zIndex: 10, color: 'rgba(255,255,255,0.5)', fontSize: '1.5rem', fontFamily: 'monospace' }}>06</span>
      ...
      <nav className="slides-navigation" id="slidesNav" style={{ position: 'absolute', right: '2rem', top: '50%', transform: 'translateY(-50%)', zIndex: 10 }}></nav>
    </main>
  );
}
